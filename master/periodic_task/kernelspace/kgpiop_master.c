// ****************************************************************
// MASTER PERIODIC TASK MODULE FOR KERNEL SPACE MEASUREMENTS
// COMPILE: gcc kgpiop_master.c -o kgpiop_master -lm -lrt   // link to math library or   gcc -Wall -w gpio1.c -std=gnu99 -lrt  
// RUN: sudo chrt -f 99 ./kgpiop_master <number-of-loops>// with realime priority 99
// The number-of-loops (as runs) must be half of the slave's (gpiop_slave)
// ****************************************************************
...
//long int *delays = (long int *) malloc(atoi(argv[1])*2 * sizeof(long int)); // desmeuei mnimi gia osa samples dosoume
...
// ******************* Trigger the other device ************************
// TRIGGER THE INPUT OF OTHER DEVICE TO PRODUCE INTERRUPT TO RUN KERNEL MODULE TIMER
write(fdout, ON, sizeof(ON));
	
gettimeofday(&initial_overall_time); /
while(choice <= atoi(argv[1])) 
{
	//close(fd); fd = open(GPIOValue_in, O_RDONLY);
	lseek(fd, 0, SEEK_SET); // consume any prior interrupt
	read(fd, buf, sizeof buf);
	//printf("\n WAITING: Input pin %d value currently is %d\n", GPIO_in, atoi(buf));
	
	poll(&pfd, 1, -1);    // wait for interrupt (READS 1)
	...
	poll(&pfd, 1, -1);    // wait for interrupt (READS 0)
	lseek(fd, 0, SEEK_SET); // consume interrupt
	read(fd, buf, sizeof buf);
    ...
    //printf("gettimeofday1: elapsed in usecs+nsecs = %ld\n", \
    ((finish_time.tv_sec *1000000 + finish_time.tv_nsec) - (start_time.tv_sec * 1000000 + start_time.tv_nsec)));    
    ...
    ((ts_end.tv_sec*1000000 + ts_end.tv_nsec/1000) - (ts_begin.tv_sec * 1000000 + ts_begin.tv_nsec/1000)));
	 ...
	 delays[i]= (ts_end.tv_sec*1000000 + ts_end.tv_nsec/1000) - (ts_begin.tv_sec * 1000000 + ts_begin.tv_nsec/1000); //se microseconds us 
